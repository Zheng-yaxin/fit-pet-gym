  ...
  <div class="feature-page">
    <section class="feature-hero">
      <div>
        ...
        <h1>课后反馈</h1>
        <p class="summary">选择已完成课程，记录评分、强度和主观体验，帮助教练和场馆优化课程。</p>
      </div>
      <el-button type="primary" :disabled="!pendingList.length" @click="submit">提交反馈</el-button>
    </section>

    <el-card shadow="never" class="panel">
      <el-form :model="form" label-width="100px">
        <el-form-item label="待反馈课程">
          <el-select v-model="selectedPendingKey" class="full" placeholder="选择已完成课程" @change="applyPending">
            <el-option
          ...
          <p v-if="!pendingList.length" class="empty-tip">暂无待反馈课程，完成课程后会出现在这里。</p>
        </el-form-item>
        ...
        <el-form-item label="反馈内容">
          <el-input v-model="form.content" type="textarea" :rows="4" placeholder="这节课哪里有帮助，哪里还可以改进？" />
        </el-form-item>
      </el-form>
    </el-card>
...
const loadList = async () => {
  ...
  if (!selectedPendingKey.value && pendingList.value.length) {
    selectedPendingKey.value = String(pendingList.value[0].scheduleId)
    applyPending()
  }
}
...
const formatPendingLabel = (item: any) => {
  ...
  return `${item.courseName || '课程'} · ${item.coachName || '教练'} · ${time || '待定'}`
}

const applyPending = () => {
  const item = pendingList.value.find((pending) => String(pending.scheduleId) === selectedPendingKey.value)
