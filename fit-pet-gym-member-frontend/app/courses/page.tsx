...
function feedbackKey(item: CourseFeedbackPending) {
  return item.bookingId ? `pt-${item.bookingId}` : `course-${item.scheduleId ?? item.enrollmentId ?? "unknown"}`;
}

function feedbackLabel(item: CourseFeedbackPending) {
  const source = item.feedbackType === "personal_training" ? "PT" : "Course";
  const coach = item.coachName ?? "Coach pending";
  const title = item.courseName ?? source;
  const time = item.classTime ?? [item.startTime, item.endTime].filter(Boolean).join(" - ");
  return `${source} · ${title} · ${coach}${time ? ` · ${time}` : ""}`;
}
...
export default function CoursesPage() {
  ...
  const [pendingFeedback, setPendingFeedback] = useState<CourseFeedbackPending[]>([]);
  const [feedbackHistory, setFeedbackHistory] = useState<CourseFeedback[]>([]);
  const [selectedFeedbackKey, setSelectedFeedbackKey] = useState("");
  const [feedbackRating, setFeedbackRating] = useState(5);
  const [feedbackIntensity, setFeedbackIntensity] = useState(6);
  ...
  const load = async () => {
    ...
    setError("");
    try {
      const [nextCourses, nextSchedules, nextEnrollments, nextCoaches, nextBookings, nextPendingFeedback, nextFeedbackHistory] = await Promise.all([
        getMemberCourses(),
        getMemberCourseSchedules(),
        getMemberEnrollments(),
        getMemberCoachList(),
      ...
      if (!selectedFeedbackKey && nextPendingFeedback?.length) {
        setSelectedFeedbackKey(feedbackKey(nextPendingFeedback[0]));
      }
  ...
  const selectedFeedback = pendingFeedback.find((item) => feedbackKey(item) === selectedFeedbackKey) ?? pendingFeedback[0];
  ...
  const handleSubmitFeedback = async (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    if (!selectedFeedback) {
      setError("No completed class is waiting for feedback.");
      return;
    }
    setBusy(true);
    setError("");
    try {
      await submitCourseFeedback({
        scheduleId: selectedFeedback.scheduleId,
        bookingId: selectedFeedback.bookingId,
        courseId: selectedFeedback.courseId,
        coachId: selectedFeedback.coachId,
    ...
    <main className="feature-page feature-motion-page feature-motion-booking" aria-label="课程预约">
      ...
      <div className="feature-shell wide">
        ...
        <section className="feature-panel">
          ...
          <div className="feature-grid two">
            <form className="feature-form" onSubmit={handleSubmitFeedback}>
              <h2>
              ...
              <label>
                Waiting item
                <select
                  value={selectedFeedback ? feedbackKey(selectedFeedback) : selectedFeedbackKey}
                  onChange={(event) => setSelectedFeedbackKey(event.target.value)}
                  disabled={!pendingFeedback.length}
                >
                  {pendingFeedback.map((item) => (
                    <option key={feedbackKey(item)} value={feedbackKey(item)}>
                      {feedbackLabel(item)}
                    </option>
                  ))}
                </select>
              ...
              <label>
                ...
                <input
                  ...
                  onChange={(event) => setFeedbackIntensity(Number(event.target.value))}
                />
              </label>
              <label>
                Notes
                <input
                  ...
                  placeholder="What helped? What should the coach adjust?"
                />
              </label>
              <button type="submit" disabled={busy || !pendingFeedback.length}>Submit feedback</button>
            </form>

            <article className="feature-list">
              <span>Waiting for feedback</span>
              {pendingFeedback.slice(0, 6).map((item) => (
                <div className="feature-row" key={feedbackKey(item)}>
                  <div>
                    <h3>{item.feedbackType === "personal_training" ? "PT review" : item.courseName ?? "Course review"}</h3>
                    <p>{feedbackLabel(item)}</p>
                  </div>
                  <button type="button" disabled={busy} onClick={() => setSelectedFeedbackKey(feedbackKey(item))}>Select</button>
                </div>
              ))}
              {!pendingFeedback.length ? <p>No completed class is waiting for feedback.</p> : null}
            </article>
          </div>

          <article className="feature-list">
            <span>My feedback status</span>
